...
async function test() {
  try {
    const t1 = await taskManager.addTask({ title: "Write docs", priority: "high" });
    const t2 = await taskManager.addTask({ title: "Fix bugs", due: "2025-01-10" });
    const t3 = await taskManager.addTask({ title: "Team meeting", priority: "low" });
